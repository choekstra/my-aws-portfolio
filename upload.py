...
def serve_static_file(event):
    print(f"*** in serve_static_file", event);
    ...
    print(f"here raw_path: {raw_path}");
    print(f"in serve static Requested file: {file_name}")  # Log the requested file name
    
    # Determine the content type based on the file extension
    content_type = "text/html"
    if file_name.endswith('.js'):
        content_type = "application/javascript"
    print(f"Content-Type is set to: {content_type}")  # Log the determined content type

    # Use the environment variable to get the bucket name for website assets
    bucket_name = os.environ['WEBSITE_ASSETS_BUCKET']
    print(f"NOW Fetching '{file_name}' from bucket '{bucket_name}'")  # Log the S3 get_object request
	...
	print(f"returning body: {file_content}");
        return {
            'statusCode': 200,
