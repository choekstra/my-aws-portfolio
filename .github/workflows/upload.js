...
document.getElementById('upload-form').addEventListener('submit', function(e) {
  ...
  if (!file) {
      alert('Pleasse select a file to upload.');
      return;
  }

  // Fetch the pre-signed URL from the Lambda function
  const contentType = file.type; 
    fetch(`https://5mmobzqcyi3iqwhpmhg3dbcmt40woent.lambda-url.us-east-1.on.aws/generate-presigned-url?content-type=${encodeURIComponent(contentType)}`) // upload FUN URL PLC HOLDER
